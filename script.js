function moneyApp() {
    ...
    // =========================
    const lastTransactions = this.transactions.slice(0, 5);

    let detail = lastTransactions.map(t => {
        return `• ${t.category}: ${format(t.amount)}`;
    }).join('\n');

    // =========================
    // TEMPLATE MESSAGE
    // =========================
    const message = `*REKAP WEALTHYFLOW PRO*
Tanggal: ${today}
----------------------------------
💰 Masuk: ${format(this.totalIncome)}
💸 Keluar: ${format(this.totalExpense)}
💼 *Saldo Akhir: ${format(this.totalSaldo)}*
...
${this.getSavingSuggestion()}

_Dikirim dari WealthyFlow Pro_`;
